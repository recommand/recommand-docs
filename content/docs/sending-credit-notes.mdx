---
title: Sending Credit Notes
description: Issue and send Peppol credit notes to reverse or adjust invoices using the Recommand API. Includes field reference and code examples.
---
...
The response uses the same [delivery fields as sending an invoice](/docs/sending-invoices#success-response). `success: true` confirms the request succeeded; check `deliveryStatus` and `deliveries` for confirmation or failure, including outcomes reported after the send returns.
