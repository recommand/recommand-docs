...
Present the `verificationUrl` to your user, for example by displaying it in your UI or redirecting them to it directly. The verification page is self-contained and works in any browser; no Recommand account is required. From there, the user can forward the link to whoever in their organisation is authorised to complete the identity check.
