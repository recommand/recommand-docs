...
<Callout type="info" title="Already using webhooks?">
  Rules are a more powerful and flexible alternative to our original webhook setup. If you only need a webhook with optional company scoping and HMAC signing, see [Working with Webhooks](/docs/working-with-webhooks).
</Callout>
...
- verification status
- reporting status
- delivery channel, delivery status and failure category
- outcome code
...
Rules run asynchronously. Deliveries in this view are attempts to execute a rule action, such as posting a webhook or sending a notification email. Document deliveries over Peppol or email are shown on the document itself; use the **Delivery status changed** trigger to react to their outcomes.
...
If a rule action delivery fails, Recommand retries it automatically for a limited number of times. You can also manually retry failed deliveries from the dashboard.
...
- Notify your team when a document delivery fails, optionally filtered by channel or failure category
