...
The examples below call the list companies endpoint, because it needs no input of
its own and shows you real data back. If you only want to know whether your
credentials work, call the [verify authentication
endpoint](/reference/authentication/verify-auth) instead: it answers
`{"success": true}` or 401 and nothing else.
