...
HTTP 422 is different: sending failed during the request and no email fallback succeeded (see [delivery errors](#delivery-errors) below).
...
If sending fails during the request and no email fallback succeeds, the response is **HTTP 422**. No document is stored and the failed attempt does not count toward your usage:
...
`deliveryFailure` identifies a Peppol failure's channel and category, such as `transport`, `recipient_not_found` or `document_not_supported`. Read `errors.root` for the accompanying message.

A failure reported after the send request succeeded does not produce a new HTTP error response. It changes the stored delivery to `failed` and emits `document.delivery_status_changed`. Fetch the document's `deliveries` for the failure category, message and provider code. A document can be `delivered` overall while another channel failed.

If you configured `email.when: "on_peppol_failure"`, a later Peppol failure can trigger the requested email fallback. Inspect the attempts before resending; `pending` means the outcome is still unknown. See [Tracking Delivery](/docs/email-delivery-and-notifications#tracking-delivery).
...
1. **Recipient's AP is unavailable**
   ...
   - Configure [email delivery](/reference/sending/send-document) as a fallback and track the email attempt separately
