...
<Callout type="info" title="Need more control than a single webhook?">
...
| `secret`    | Secret used to sign deliveries with HMAC SHA-256 | No       | `"your_webhook_signing_secret"`            |

## Verifying Webhook Signatures

When you configure a `secret`, every delivery includes an `X-Signature` header in this format:
...
<Callout type="warn" title="Keep the raw body unchanged">
...
This event confirms acceptance for sending. It also includes `deliveryStatus`, which can still be `pending`; a later failure is reported separately. Older queued events may have `deliveryStatus: null`.

### Document Delivery Status Changed Event

Fired when an individual delivery is confirmed or fails, or when a deferred email fallback creates a new attempt. Configure a **Delivery status changed** rule under **Webhooks and rules** to send this event to your endpoint.
...
`channel` is `peppol` or `email`. Statuses are `pending`, `delivered` and `failed`; `previousStatus` is `null` for a newly created deferred fallback attempt. `failure` is `null` when no failure applies. Failure categories are `recipient_not_found`, `document_not_supported`, `validation`, `transport`, `recipient_rejected`, `duplicate` and `other`.

A document can have several deliveries. Fetch its `deliveries` and `deliveryStatus` to see the combined outcome; one failed channel does not imply every channel failed. Handle webhook retries idempotently. See [Tracking Delivery](/docs/email-delivery-and-notifications#tracking-delivery) for status semantics and fallback behaviour.

### Report Status Changed Event

Fired when a French e-reporting report's `reportingStatus` or `outcomeCode`
changes. A report can become `filed` before its final outcome arrives. Further
events can therefore have the same reporting status and a different outcome.
...
- `previousOutcomeCode`: the outcome before this change, or `null` when none was known
- `final`: true for `filed` or `filed_rectificative` with outcome `300`, or for `superseded` or `rejected`
...
A filed report normally moves from an empty outcome through `500` to `300`.
For an outcome-only change, `previousReportingStatus` equals `reportingStatus`.
For example, the final event can carry `reportingStatus: "filed"`,
`previousReportingStatus: "filed"`, `previousOutcomeCode: "500"`,
`outcomeCode: "300"` and `final: true`.

For successful completion, require a filed status and outcome `300`.
`final: true` alone also covers rejection and cancellation or replacement.
Route outcome `501` to support with the report reference and period. Handle
webhook retries idempotently and avoid repeating completion actions when several
events have status `filed`.
