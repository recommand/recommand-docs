...
<Callout type="info" title="Need more control than a single webhook?">
  Webhooks are part of Recommand's broader rule system. If you want to filter by event details, send emails, combine multiple actions, configure HMAC signing, or manage delivery retries from one place, see [Working with Rules](/docs/rules).
</Callout>
