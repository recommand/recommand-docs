...
identifier. These references can be `null` when unavailable, including when no
Peppol transmission took place. Keep available references to trace a transmission.

`success: true` means the send request succeeded; the receiving channel may still need to confirm delivery. In this example, the Peppol attempt is `pending`. Use `deliveries` for each attempt and `deliveryStatus` for the combined outcome. Follow later changes through [delivery status webhooks](/docs/working-with-webhooks#document-delivery-status-changed-event) or by fetching the document.

`sentOverEmail` and `emailRecipients` describe email sent during this request, not confirmation by the recipient's mail server. See [Tracking Delivery](/docs/email-delivery-and-notifications#tracking-delivery) for status meanings, email delivery and fallback after a later Peppol failure.
