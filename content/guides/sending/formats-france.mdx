...
Name the format with `doctypeId` on the send request. Without it you send plain
Peppol BIS 3 UBL, which is valid on the network but not what the French reform
asks for inside its perimeter.
