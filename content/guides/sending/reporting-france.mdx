...
### Name the invoicing framework

`billingMode` is required on every cross-border invoice report. It is the
invoicing framework of the document you are reporting, in the same codes and
with the same meaning as the `billingMode` of a French regulated invoice, so an
integration that already sends French invoices has the value at hand.
...
These are the frameworks confirmed for cross-border reporting. Other codes an
invoice can carry are not supported on a report today and are refused with a
400.

<Callout type="warn" title="The framework is checked after the report is accepted">
  The framework is checked against the invoice when the filing for the period is
  assembled, which happens after your report has been accepted. A framework that
  does not match the invoice can cause that filing to be rejected, and the
  rejection reaches every report on the same filing. Take the value from the
  invoice rather than sending a fixed one.
</Callout>
...
Both are matched on the data that identifies the operation, never on the
reference:

| Report | Matched on |
| --- | --- |
| B2C sales | The day, the category and the currency |
| B2C payments | The day and the currency |
| Cross-border invoice | The document number |
| Cross-border payment | The `invoiceNumber` of the invoice it belongs to |

Dates are not part of how a cross-border report is matched: an invoice report
is matched on its document number without its issue date, and a payment report
on the invoice number without its payment date. Every match is scoped to the
reporting company, so two companies using the same invoice numbers never
collide.

A correction **replaces** the report it matches rather than merging into it, so
send the complete report and not only the fields that changed. A cancellation
carries the complete report as well.

<Callout type="warn" title="Corrections have a deadline">
  A correction and a cancellation are both refused once the filing for that
  period has been assembled. That is not the same moment as `periodEnd`: the
  filing is put together after the period closes, so the window to correct
  extends past the last day of the period and then ends. Submit corrections as
  soon as you find them rather than at the end of the period.
</Callout>

Reusing the original reference for a correction does not correct anything: the
reference is the idempotency key, so the earlier report comes back with
`duplicate: true` and nothing is filed. When that happens the response also
carries a `warning` saying what was found on file, so a correction that quietly
did nothing can be spotted in the response rather than at the end of the
period. A plain retry of an identical request has no `warning`.
...
| 400 | A cross-border invoice report left out `billingMode` or used a code that is not supported for reporting | Send the invoicing framework of the invoice, from the table above |
...
### Outcome codes belong to the filing

`outcomeCode` is the tax administration's answer about the **filing**, not
about the single report you are looking at. Every report carried on the same
filing therefore shows the same code, and one rejected filing means the whole
period needs attention rather than one report.

| Code | Meaning |
| --- | --- |
| `300` | Déposée: the filing was deposited. |
| `500` | Recevable: the filing was accepted. |
| `501` | Irrecevable: the filing was refused. |

The raw code is what the field carries, and it stays as it is. A code that is
not in this table is passed through unchanged rather than translated or hidden,
so treat the field as an open set: match on the codes you know and fall back on
